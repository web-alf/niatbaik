...
func (r *InvoiceRepo) MarkGoogleAdsPollRetryable(ctx context.Context, id uuid.UUID, summary string, due time.Time) error {
	return affected(r.db.WithContext(ctx).Model(&model.Invoice{}).Where("id = ? AND google_ads_server_status = ?", id, model.GoogleAdsConversionProcessing).Updates(map[string]any{"google_ads_server_status": model.GoogleAdsConversionRetryable, "google_ads_server_error": summary, "google_ads_server_next_attempt_at": due, "google_ads_server_processing_at": nil, "google_ads_poll_attempt_count": gorm.Expr("google_ads_poll_attempt_count + 1")}), "mark poll retryable")
}
func (r *InvoiceRepo) MarkGoogleAdsPollFailed(ctx context.Context, id uuid.UUID, summary string) error {
	return affected(r.db.WithContext(ctx).Model(&model.Invoice{}).Where("id = ? AND google_ads_server_status = ?", id, model.GoogleAdsConversionProcessing).Updates(map[string]any{"google_ads_server_status": model.GoogleAdsConversionFailed, "google_ads_server_error": summary, "google_ads_server_next_attempt_at": nil, "google_ads_server_processing_at": nil, "google_ads_poll_attempt_count": gorm.Expr("google_ads_poll_attempt_count + 1")}), "mark poll failed")
}
func (r *InvoiceRepo) MarkGoogleAdsAcceptedUntracked(ctx context.Context, id uuid.UUID, summary string) error {
	return affected(r.db.WithContext(ctx).Model(&model.Invoice{}).Where("id = ? AND google_ads_server_status = ?", id, model.GoogleAdsConversionProcessing).Updates(map[string]any{"google_ads_server_status": model.GoogleAdsConversionAcceptedUntracked, "google_ads_server_error": summary, "google_ads_server_next_attempt_at": nil, "google_ads_server_processing_at": nil}), "mark accepted untracked")
}
...
func (r *InvoiceRepo) RetryGoogleAdsInvoice(ctx context.Context, id uuid.UUID, snap GoogleAdsSnapshot, now time.Time) error {
	return affected(r.db.WithContext(ctx).Model(&model.Invoice{}).Where("id = ? AND is_paid = ? AND google_ads_server_status NOT IN ? AND (gclid <> '' OR gbraid <> '' OR wbraid <> '')", id, true, []string{model.GoogleAdsConversionServerSent, model.GoogleAdsConversionAcceptedUntracked}).Updates(map[string]any{"google_ads_customer_id_snapshot": snap.CustomerID, "google_ads_login_customer_id_snapshot": snap.LoginCustomerID, "google_ads_conversion_action_id_snapshot": snap.ConversionActionID, "google_ads_server_upload_enabled_snapshot": snap.Enabled, "google_ads_server_status": model.GoogleAdsConversionPendingUpload, "google_ads_server_request_id": "", "google_ads_server_error": "", "google_ads_server_attempt_count": 0, "google_ads_poll_attempt_count": 0, "google_ads_server_attempted_at": nil, "google_ads_server_sent_at": nil, "google_ads_server_next_attempt_at": now, "google_ads_server_processing_at": nil}), "retry invoice")
}
