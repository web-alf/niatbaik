...
function DashboardView() {
  ...
  const { txns, dailyDonations, campaignSeed } = window.NB;
  ...
  const [tw, setTw] = useTweaks(window.NB_TWEAKS);
  ...
  // Admin dashboard
  const stats = [
    { icon:'wallet', label:'Total Donasi',         value: fmtIDRShort(1_842_315_500), delta:'+18.4%', sub:'sepanjang waktu', accent:'brand' },
    { icon:'creditcard', label:'Total Transaksi',  value: fmtNum(24_812), delta:'+12.1%', sub:'transaksi sukses', accent:'sky' },
    { icon:'megaphone', label:'Campaign Aktif',    value: '14', delta:'+3', deltaTone:'up', sub:'sedang berjalan', accent:'ok' },
    { icon:'handshake', label:'Total Fundraiser',  value: '38', delta:'+5', sub:'mitra aktif', accent:'warn' },
    { icon:'target', label:'Leads dari Iklan',     value: fmtNum(13_072), delta:'+22.6%', sub:'30 hari terakhir', accent:'brand' },
    { icon:'bolt',  label:'Conversion Rate',       value: '4.8%', delta:'+0.6pp', sub:'lead → donasi', accent:'sky' },
    { icon:'sun',   label:'Donasi Hari Ini',       value: fmtIDRShort(34_280_000), delta:'+8.1%', sub:'182 transaksi hari ini', accent:'ok' },
    { icon:'flame', label:'Donasi Bulan Ini',      value: fmtIDRShort(412_460_000), delta:'+14.3%', sub:'Mei 2026', accent:'bad' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <DashboardTweaks tw={tw} setTw={setTw}/>
...
// ----- Tweaks panel for Recent Transactions table -----
function DashboardTweaks({ tw, setTw }) {
  return (
    <TweaksPanel title="Tweaks · Transaksi Terbaru">
      <TweakSection label="Payment Gateway"/>
      <TweakToggle label="Auto-confirm Moota/Flip" value={tw.txnAutoConfirmMoota}
                   onChange={(v) => setTw('txnAutoConfirmMoota', v)}/>
      <TweakToggle label="Highlight baris auto-confirm" value={tw.txnHighlightAutoConfirm}
                   onChange={(v) => setTw('txnHighlightAutoConfirm', v)}/>
      <div style={{ fontSize: 10.5, lineHeight: 1.45, color: 'rgba(41,38,27,.6)', margin: '-2px 0 4px' }}>
        VA Moota (BCA/Mandiri/BNI) &amp; Flip: status Pending otomatis menjadi Paid via webhook.
      </div>

      <TweakSection label="Toolbar"/>
      <TweakToggle label="Search & filter aktif" value={tw.txnShowToolbar}
                   onChange={(v) => setTw('txnShowToolbar', v)}/>
      <TweakToggle label="Filter metode pembayaran" value={tw.txnShowMethodFilter}
                   onChange={(v) => setTw('txnShowMethodFilter', v)}/>

      <TweakSection label="Tampilan"/>
      <TweakSlider label="Jumlah baris" value={tw.txnRowCount} min={3} max={15} step={1}
                   onChange={(v) => setTw('txnRowCount', v)}/>
      <TweakRadio  label="Density" value={tw.txnDensity}
                   options={['compact','regular','comfy']}
                   onChange={(v) => setTw('txnDensity', v)}/>
      <TweakToggle label="Striped rows" value={tw.txnStriped}
                   onChange={(v) => setTw('txnStriped', v)}/>
      <TweakColor  label="Aksen invoice" value={tw.txnAccent}
                   options={['#2E4191','#38B6FF','#16A34A','#F59E0B','#DC2626']}
                   onChange={(v) => setTw('txnAccent', v)}/>

      <TweakSection label="Kolom"/>
      <TweakToggle label="Donatur" value={tw.txnShowDonatur}
                   onChange={(v) => setTw('txnShowDonatur', v)}/>
      <TweakToggle label="Campaign" value={tw.txnShowCampaign}
                   onChange={(v) => setTw('txnShowCampaign', v)}/>
      <TweakToggle label="Metode" value={tw.txnShowMetode}
                   onChange={(v) => setTw('txnShowMetode', v)}/>
      <TweakToggle label="Sumber" value={tw.txnShowSumber}
                   onChange={(v) => setTw('txnShowSumber', v)}/>
      <TweakToggle label="Status" value={tw.txnShowStatus}
                   onChange={(v) => setTw('txnShowStatus', v)}/>
      <TweakToggle label="Tanggal" value={tw.txnShowTanggal}
                   onChange={(v) => setTw('txnShowTanggal', v)}/>

      <TweakSection label="Privasi & Copy"/>
      <TweakToggle label="Anonim semua donatur" value={tw.txnAnonymizeAll}
                   onChange={(v) => setTw('txnAnonymizeAll', v)}/>
      <TweakText   label="Subtitle" value={tw.txnTitle}
                   onChange={(v) => setTw('txnTitle', v)}/>
    </TweaksPanel>
  );
}
