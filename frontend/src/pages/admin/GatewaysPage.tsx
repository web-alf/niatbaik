...
// Small status pill.
function GwBadge({ ok, on = 'Aktif', off = 'Nonaktif' }: any) {
  return (
...
function GwRow({ label, value, good }: any) {
  return (
...
function WebhookProbe({ label, url, probe }: any) {
  const [copied, setCopied] = useState(false);
  const [testing, setTesting] = useState(false);
  const [result, setResult] = useState<any>(null); // {ok, text}
  ...
  const test = async () => {
    ...
    catch (e: any) { setResult({ ok: false, text: 'Gagal: ' + (e?.message || 'error') }); }
    setTesting(false);
  };
...
export default function GatewaysPage() {
  ...
  const storeSettings = useDataStore((s) => s.settings);
  const [settings, setSettings] = useState<any>(storeSettings || null);
  ...
  // Flip transactions
  const [flipTxns, setFlipTxns] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive && res?.data) { setSettings(res.data); useDataStore.setState({ settings: res.data }); }
      } catch {}
    ...
    (async () => {
      try {
        ...
        const res = await api.invoices('limit=50');
        const rows = (res && res.data) || [];
        const list = Array.isArray(rows) ? rows : [];
        if (alive) setFlipTxns(list.filter((t: any) => String(t.type_payment || '').toLowerCase() === 'flip').slice(0, 8));
  ...
  return (
    <div className="p-4 lg:p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-mute mt-1">Status koneksi, webhook, dan aktivitas gateway pembayaran. Konfigurasi diubah di <button onClick={() => navigate('/settings')} className="font-semibold text-brand-600 hover:underline">Settings → Payment</button>.</p>
      </div>
