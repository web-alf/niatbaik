...
// DonationInvoicePage: Standalone checkout/invoice status page for checking out
// directly on niatbaik.org instead of relying on external payment gateway screens.
function DonationInvoicePage({ invoiceNumber, onBack }) {
  const [dark, toggleDark] = usePublicDark();
  const [invoice, setInvoice] = useState(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await window.api.paymentStatus(invoiceNumber);
        if (!alive) return;
        if (!res || !res.data) {
          setState('notfound');
          return;
...
window.DonationInvoicePage = DonationInvoicePage;
