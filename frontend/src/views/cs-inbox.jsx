function CSInboxView() {
  // Live invoices when API loaded them; else seed.
  const seedTxns = (window.TRANSACTIONS && window.TRANSACTIONS.length) ? window.TRANSACTIONS : window.NB.txns;
  const { setInvoiceTxn, showToast } = useApp();

  // local stateful copy so we can mutate status
  const [txns, setTxns] = useStateA(() => seedTxns.map(t => ({ ...t })));
  const [filter, setFilter] = useStateA('all');
  const [search, setSearch] = useStateA('');
  const [selected, setSelected] = useStateA(txns[1]);
  const [advFilterOpen, setAdvFilterOpen] = useStateA(false);
  ...
  const filtered = useMemoA(() => txns.filter(t => {
    ...
    if (adv.range?.start) {
      const d = window.parseTxnDate(t.date);
      if (d) {
...
function ActiveFilterChips({ adv, onChange }) {
  ...
  if (adv.range?.start) chips.push({ k:'rng', l:`Tanggal: ${window.formatRangeLabel(adv.range.start, adv.range.end)}`, remove: () => onChange({ ...adv, range:null }) });
...
function AdvFilterModal({ open, onClose, value, onChange, txns }) {
  ...
  useEffectA(() => { if (open) setDraft(value); }, [open]);

  const allMethods = useMemoA(() => Array.from(new Set(txns.map(t => t.method))), [txns]);
  const allSources = useMemoA(() => Array.from(new Set(txns.map(t => t.utm.source))), [txns]);
      ...
      <div className="space-y-5">
        ...
        <Group label="Atribut Donasi">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <Toggle value={draft.anonOnly}   onChange={(v) => setDraft({ ...draft, anonOnly:v })}   label="Hanya donasi anonim" sub="Filter Hamba Allah"/>
            <Toggle value={draft.hasMessage} onChange={(v) => setDraft({ ...draft, hasMessage:v })} label="Punya doa/pesan"     sub="Skip yang kosong"/>
            <Toggle value={draft.hasNote}    onChange={(v) => setDraft({ ...draft, hasNote:v })}    label="Punya catatan CS"    sub="Sudah ada notes"/>
          </div>
        </Group>
      </div>
    </Modal>
...
function ExportLimitedModal({ open, onClose, rows, showToast }) {
  ...
  const [format, setFormat] = useStateA('csv');
  const [limit, setLimit]   = useStateA('filtered');  // 'filtered' | 100 | 500 | 1000
  ...
  const doExport = () => {
    ...
    if (format === 'csv') exportCSV(out, 'cs_inbox', window.__nb_dateRange);
    else exportExcel(out, 'cs_inbox', window.__nb_dateRange, 'CS Inbox');
...
function CSDetail({ t, onOpen, onCopy, onMarkPaid, showToast }) {
  ...
  const [waVariant, setWaVariant] = useStateA('default');
  const [waMessage, setWaMessage] = useStateA(templates(false));
  useEffectA(() => { setWaMessage(templates(waVariant === 'paid')); /* eslint-disable-next-line */ }, [t.id, waVariant]);
...
window.CSInboxView = CSInboxView;
