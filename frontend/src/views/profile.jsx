...
function ProfileView() {
  ...
  const save = async () => {
    ...
    try {
      await window.api?.updateProfile?.({ name: patch.name, email: patch.email, phone: patch.wa, avatar: patch.avatar });
  ...
  // -------- Password state --------
  const [pwd, setPwd]         = useStateA({ old: '', neu: '', con: '' });
  const [pwdErrors, setPwdErrors] = useStateA({});
  const [pwdShow, setPwdShow] = useStateA({ old:false, neu:false, con:false });

  const pwdStrength = useMemoA(() => {
    const p = pwd.neu;
